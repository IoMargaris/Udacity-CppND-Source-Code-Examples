 ...
 * @create date 27 Aug. 2020 -13:17:00

 * @desc [The problem with the previous example is that we can only lock the mutex once and the only way 
 * to control lock and unlock is by invalidating the scope of the std::lock_guard object.
 * 
 ...
 * In the example, the lock is released before some non-critical work is performed (simulated by sleep_for) and re-engaged before some other work is 
 * performed in the critical section and thus under the lock again at the end of the function. This is particularly useful for optimizing performance 
 * and responsiveness when a significant amount of time passes between two accesses to a critical resource
 * 
 ...
 * 
 * Despite the advantages of std::unique_lock<> and std::lock_guard over accessing the mutex directly, however, the deadlock situation where two mutexes are accessed 
 * simultaneously (see the last section) will still occur
 * ]
 */
